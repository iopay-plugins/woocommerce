...
(function (root, factory) {
  ...
  function registerEvents (ele, eventCollection, options) {
    const events = getKeys(eventCollection)
    let e
  ...
  function format (cardnumber, format) {
    ...
    const cn = cardnumber.split('')
    let val = [], i = 0, currNo = 0
  ...
  /**
     ...
     */
  function mask (val) {
    const ret = new Array(val.length - (val.slice(-3).length))
  ...
  ccmask.onBlur = {
    ...
    keypress: function (e) {
      const value = String.fromCharCode(e.charCode || e.which)
    ...
    blur: function (e) {
      const value = $(this).data('unmaskedValue'),
  ...
  ccmask.onKeyUp = {
    ...
    keypress: function (e) {
      let value = String.fromCharCode(e.charCode || e.which)
    ...
    paste: function () {
      ...
      setTimeout(function () {
        const value = ele.value.replace(/[^0-9]+/, '').substring(0, 16)
        $(ele).val(value)
